...
function oauthErrorMessage(code: string | null): string | null {
  ...
  if (lower.includes("oauth") || lower === "callback" || lower === "oauthcallbackerror") {
    return (
      "No se pudo completar el inicio de sesión con Google (cookie de seguridad). " +
      "Probá de nuevo en esta misma ventana; si sigue fallando, el sitio suele tener " +
      "NEXTAUTH_URL distinto del dominio que estás usando (www vs sin www). En Railway debe coincidir con la URL real."
    )
  }
  if (lower.includes("accessdenied")) return "Acceso denegado en Google."
  return `No se pudo iniciar sesión (${code}). Probá con correo y contraseña o contactá soporte.`
}
...
function LoginForm() {
  ...
  const callbackUrl = "/"
  const oauthErrCode = searchParams.get("error")
  ...
  const urlOAuthHint = oauthErrorMessage(oauthErrCode)
        ...
        <div className="w-full max-w-[440px]">
          ...
          <div className="bg-white rounded-2xl shadow-[0_25px_50px_-12px_rgba(0,0,0,0.45)] border border-slate-200/80 overflow-hidden">
            <div className="px-8 pt-8 pb-2 text-center">
              ...
              <p className="text-sm text-slate-500 mt-2">Entrá con Google o con tu correo registrado</p>
            </div>
            ...
            <div className="px-8 pb-8 pt-4 space-y-4">
              ...
              <Button
                type="button"
                variant="outline"
                className="w-full h-12 font-semibold border-slate-200 hover:bg-slate-50 text-slate-800"
                onClick={handleGoogleSignIn}
                disabled={googleLoading}
              >
                {googleLoading ? (
                  <span className="flex items-center gap-2">
                    <svg className="animate-spin h-5 w-5 text-slate-600" viewBox="0 0 24 24" aria-hidden>
              ...
              <div className="relative py-1">
                ...
                <span className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-white px-3 text-xs font-medium text-slate-400 uppercase tracking-wider">
                  o con email
                </span>
              </div>
