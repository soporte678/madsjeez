...
import { getHelpArticle, allHelpSlugs, type HelpArticle } from "@/data/help-articles";
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!a) return {};
  const url = `${SITE_URL}/ayuda/${a.slug}`;
  return {
    title: a.seoTitle,
    description: a.metaDescription,
    alternates: { canonical: `/ayuda/${a.slug}` },
...
export default async function AyudaArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const url = `${SITE_URL}/ayuda/${a.slug}`;
  const related = a.related.map((s) => getHelpArticle(s)).filter((x): x is HelpArticle => Boolean(x)).slice(0, 3);

  const breadcrumb = [
    { name: "Inicio", url: SITE_URL },
    { name: "Ayuda", url: `${SITE_URL}/ayuda` },
    { name: a.title, url },
  ];
  const articleLd = {
    ...
    <main className="bg-background text-foreground">
      ...
      <nav aria-label="Migas de pan" className="mx-auto max-w-3xl px-4 pt-6">
        <ol className="flex flex-wrap items-center gap-1 text-xs text-muted-foreground">
          ...
          <li><Link href="/ayuda" className="hover:text-foreground">Ayuda</Link></li>
          <li aria-hidden><ChevronRight className="h-3.5 w-3.5" /></li>
      ...
      <article className="mx-auto max-w-3xl px-4 pb-10 pt-6">
        <span className="inline-flex items-center gap-1.5 rounded-full bg-primary/10 px-2.5 py-1 text-xs font-semibold text-primary"><LifeBuoy className="h-3.5 w-3.5" /> {a.category}</span>
        ...
        <div className="mt-10 rounded-2xl border border-border bg-gradient-to-br from-primary/10 to-transparent p-6">
          <p className="text-sm text-muted-foreground">¿No encontraste lo que buscabas? <Link href="/ayuda" className="font-semibold text-primary hover:underline">Volvé al centro de ayuda</Link> o <Link href="/search" className="font-semibold text-primary hover:underline">explorá productos</Link>.</p>
        </div>
      </article>
        ...
        <section className="border-t border-border">
          <div className="mx-auto max-w-3xl px-4 py-10">
            ...
            <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2">
              {related.map((r) => (
                <Link key={r.slug} href={`/ayuda/${r.slug}`} className="group flex items-center justify-between gap-3 rounded-xl border border-border bg-card p-4 transition hover:border-primary">
