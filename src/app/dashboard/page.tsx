...
import { 
  ...
  Bell, Search, MapPin, Sparkles, Menu, Package, DollarSign
} from "lucide-react"
...
export default function DashboardPage() {
  ...
  const { user, stats, products, orders, isLoading, error } = useDashboardData()
      ...
      <div className="max-w-full mx-auto flex relative">
        ...
        <main className="flex-1 p-8">
              ...
              <CardContent>
                <div className="flex items-center gap-2 mb-3">
                  <span className={`text-xl font-black uppercase ${
                    user?.reputationColor === "VERDE_OSCURO" ? "text-emerald-600" :
                    user?.reputationColor === "VERDE" ? "text-green-600" :
                    user?.reputationColor === "AMARILLO" ? "text-yellow-600" :
                    user?.reputationColor === "NARANJA" ? "text-orange-600" :
                    "text-red-600"
                  }`}>
                    {reputationText}
                  </span>
                  <div className={`w-2 h-2 rounded-full animate-pulse ${
                    user?.reputationColor === "VERDE_OSCURO" ? "bg-emerald-500" :
                    user?.reputationColor === "VERDE" ? "bg-green-500" :
                    user?.reputationColor === "AMARILLO" ? "bg-yellow-500" :
                    user?.reputationColor === "NARANJA" ? "bg-orange-500" :
                    "bg-red-500"
                  }`}></div>
                </div>
                <div className="h-1.5 w-full flex gap-0.5 rounded overflow-hidden mb-2">
                  ...
                  <div className={`flex-1 ${user?.reputationColor === "VERDE" ? "bg-green-500" : "bg-lime-300"}`}></div>
              ...
              <CardContent className="p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xl font-black text-slate-800">{stats.itemsToImprove}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              <CardContent className="p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs text-slate-500 font-bold uppercase">Vistas</p>
                    <p className="text-xl font-black text-slate-800">{stats.monthlyViews}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs text-slate-500 font-bold uppercase">Órdenes</p>
          ...
          <Card className="mb-8">
            ...
            <CardContent className="p-0">
              <div className="divide-y divide-slate-50">
                {[
                  { label: "Preguntas", value: stats.pendingQuestions === 0 ? "Sin pendientes" : `${stats.pendingQuestions} pendientes`, alert: stats.pendingQuestions > 0 },
                  { label: "Publicaciones activas", value: `${stats.itemsToImprove} productos`, alert: false },
                  { label: "Gestión de envíos Express", value: stats.expressShipping === 0 ? "Sin productos" : `${stats.expressShipping} productos`, alert: stats.expressShipping > 0 },
                  { label: "Precios competitivos", value: stats.competitivePrices === 0 ? "Sin sugerencias" : `${stats.competitivePrices} sugerencias`, alert: stats.competitivePrices > 0 },
                ].map((p, i) => (
          ...
          {orders.length > 0 && (
            <Card className="mb-8">
              <CardHeader className="bg-slate-50/50 border-b border-slate-100">
                <CardTitle className="text-[12px] font-black uppercase tracking-widest text-slate-500">Órdenes recientes</CardTitle>
              </CardHeader>
              <CardContent className="p-0">
                <div className="divide-y divide-slate-50">
                  {orders.slice(0, 5).map((order: any) => (
                    <div key={order.id} className="p-4 flex items-center justify-between hover:bg-slate-50">
                      <div>
                        <p className="font-bold text-sm text-slate-700">Orden #{order.id.slice(-6)}</p>
        ...
        <aside className="w-80 bg-white min-h-[calc(100vh-110px)] border-l border-slate-200 sticky top-[110px] hidden xl:block p-6 flex-shrink-0">
          <div className="space-y-8">
            ...
            <div className="border-b border-slate-100 pb-6">
              ...
              <div className="text-2xl font-black text-slate-800">{formatCurrency(stats.monthlyRevenue)}</div>
            ...
            <div>
              ...
              <div className="flex items-center gap-4">
                <div className="text-3xl font-black text-slate-800">{stats.adsSales}</div>
