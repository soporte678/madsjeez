...
- [ ] **P3009 + `20260504174800_add_access_key`:** más redeploys **no** lo arreglan solos. Con imagen **`migrate.mjs 20260514f`+**, en arranque suele **auto-ejecutarse** `migrate resolve --applied` o `--rolled-back` según el probe de `users.access_key`, y luego reintenta `migrate deploy`. Desactivar ese comportamiento: `PRISMA_MIGRATE_AUTO_RESOLVE_ACCESS_KEY=0`. Si sigue P3009 o imagen vieja: URI **Session** `:5432` (log `migrate deploy → …pooler…:5432` o Supabase → Connect → Session), en la raíz del repo:
  - Si en logs aparece `access_key existe=true` (sin auto-resolve): `DATABASE_URL='…' npm run migrate:resolve:access-key-applied`
  - Si `existe=false` o sin diagnóstico: comprobá `information_schema.columns` para `users` / `access_key`; si no hay columna: `DATABASE_URL='…' npm run migrate:resolve:access-key-rolled-back` y redeploy.
  - **Después:** redeploy en Railway y confirmá que el primer `migrate deploy` **no** muestra P3009. Doc: https://pris.ly/d/migrate-resolve
