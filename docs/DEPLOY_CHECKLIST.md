...
- [ ] **P3009 / P3018 (deriva DDL):** más redeploys **no** arreglan solos el estado en `_prisma_migrations`. Con **`migrate.mjs 20260514g`+**, en arranque suele auto-ejecutarse `migrate resolve` para casos conocidos: columna `users.access_key` (`add_access_key`), tabla `seller_meli_oauth` (`add_seller_meli_oauth`, también si aparece **P3018** `relation … already exists`). Desactivar: `PRISMA_MIGRATE_AUTO_RESOLVE_P3009_DRIFT=0` (alias: `PRISMA_MIGRATE_AUTO_RESOLVE_ACCESS_KEY=0`). Manual con URI Session `:5432`: `npm run migrate:resolve:access-key-applied` / `migrate:resolve:meli-oauth-applied` (o `-rolled-back` si el objeto **no** existe en DB). **Después:** redeploy y confirmá que el primer `migrate deploy` no muestra P3009/P3018 para esa migración. Doc: https://pris.ly/d/migrate-resolve
