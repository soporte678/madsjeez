...
- [ ] **P3009 / P3018 (deriva DDL):** más redeploys **no** arreglan solos el estado en `_prisma_migrations`. Con **`migrate.mjs 20260514h`+**, en arranque suele auto-ejecutarse `migrate resolve` para casos conocidos: `users.access_key`, `seller_meli_oauth`, enum/tabla **`MeliAdsChangeOutcome` / `meli_ads_snapshots`** (`add_meli_ads_history`; también P3018 si el enum ya existe). Desactivar: `PRISMA_MIGRATE_AUTO_RESOLVE_P3009_DRIFT=0` (alias: `PRISMA_MIGRATE_AUTO_RESOLVE_ACCESS_KEY=0`). Manual (URI Session `:5432`): scripts `migrate:resolve:*` en `package.json`. **No** borres `_prisma_migrations` salvo plan de recuperación explícito (riesgo alto). Doc: https://pris.ly/d/migrate-resolve
