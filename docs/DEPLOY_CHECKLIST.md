...
- [ ] **Bot WhatsApp:** segundo servicio **Evolution API** en Railway (Postgres + Redis + volume). Guía: [railway-evolution-setup.md](./railway-evolution-setup.md). Variables en Madsjeez: `EVOLUTION_API_URL` = dominio del servicio Evolution (no www.madsjeez.com.ar), `EVOLUTION_API_KEY`, `EVOLUTION_WEBHOOK_SECRET`. Ollama opcional en prod. Webhook: `https://<dominio>/api/webhooks/evolution`.
